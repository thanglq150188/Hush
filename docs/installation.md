...
**Lưu ý**: Tier `full` có dung lượng lớn (~2GB+), chỉ cài khi cần local inference với HuggingFace models.
...
- `${VAR}` - bắt buộc, warning nếu không set
- `${VAR:default}` - optional, dùng `default` nếu không set
...
- [Workflow đầu tiên](tutorials/01-first-workflow.md) - Tutorial chi tiết
